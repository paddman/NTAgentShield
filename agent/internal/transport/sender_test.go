...
func TestSenderDeliversSignedEventOverMutualTLS(t *testing.T) {
	files, clientKey, serverCertificate, clientPool := testPKI(t)
...
func testPKI(t *testing.T) (testPKIFiles, ed25519.PrivateKey, tls.Certificate, *x509.CertPool) {
	t.Helper()
	dir := t.TempDir()
	now := time.Now().UTC()
	...
	return testPKIFiles{caCert: caPath, clientCert: clientCertPath, clientKey: clientKeyPath}, clientKey, serverPair, clientPool
}
